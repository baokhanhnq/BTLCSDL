...
int main(void)
{
  ...
  SystemConfig_t sys_config = {
      .warning_distance = 50,          // Warn driver at 50 cm
      .danger_distance = 20,           // Trigger AEB at 20 cm
  ...
  while (1)
  {
      // Execute the safety monitoring process (measurements, median filtering, state machine)
      FCW_AEB_Process(&fcw_aeb_ctx);
      
      // Safety cycle delay (50ms) to pace ultrasonic sensor ping emission rate
      HAL_Delay(50);
    /* USER CODE END WHILE */
